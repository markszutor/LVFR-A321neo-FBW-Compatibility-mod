...
root_dir = "./dist/zLVFR-A21N-FBW-Compatibility/"
root_ctrl_excl = [root_dir+"layout.json", root_dir+"manifest.json"]
project_directories = [root_dir, root_dir+"effects", root_dir+"html_ui", root_dir+"SimObjects", root_dir+"ModelBehaviorDefs"]
...
for project_directory in project_directories:
    if (project_directory is root_dir):
        for file_name in os.listdir(project_directory):
            file_name = project_directory + file_name
...
if content_entries:
    file = root_dir+"layout.json"
    with open(file, "w") as layout_file:
        json.dump(layout_entries, layout_file, indent=2)
        print("Layout file updated.")
else:
